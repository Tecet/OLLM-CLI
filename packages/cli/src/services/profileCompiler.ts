...
export class ProfileCompiler {
  ...
  constructor() {
    ...
    this.masterDbPath = join(process.cwd(), 'packages', 'cli', 'src', 'config', 'LLM_profiles.json');
  ...
  private async loadMasterDatabase(): Promise<ProfilesData> {
    try {
      ...
      if (process.env.OLLM_LOG_LEVEL === 'debug') {
        console.log(`[ProfileCompiler] Loaded master database with ${data.models?.length || 0} models`);
      }
  ...
  private async getInstalledModels(): Promise<string[]> {
    try {
      ...
      if (process.env.OLLM_LOG_LEVEL === 'debug') {
        console.warn('[ProfileCompiler] Failed to query Ollama:', error instanceof Error ? error.message : 'Unknown error');
      }
      return [];
    }
  }
    ...
    for (const modelId of installedModelIds) {
      ...
      if (masterEntry) {
        ...
        if (unknownTemplate) {
          ...
          if (process.env.OLLM_LOG_LEVEL === 'debug') {
            console.warn(`[ProfileCompiler] Unknown model "${modelId}" - using fallback template (user-unknown-model)`);
          }
        } else {
          // No template available - log error
          console.error(`[ProfileCompiler] No "user-unknown-model" template found in master DB for unknown model "${modelId}"`);
        }
      }
    }
  ...
  private mergeProfiles(newProfiles: LLMProfile[], existing: CompiledProfile | null): LLMProfile[] {
    ...
    const existingMap = new Map(
      existing.models.map(profile => [profile.id, profile])
    );
  ...
  private async saveUserProfiles(profiles: LLMProfile[]): Promise<void> {
    try {
      ...
      if (process.env.OLLM_LOG_LEVEL === 'debug') {
        console.log(`[ProfileCompiler] Saved ${profiles.length} profiles to ${this.userProfilePath}`);
      }
    } catch (error) {
