...
function getModelHelpMessage(): string {
  return (
    'Usage: /model <list|use|pull|delete|info|keep|unload|help> [args]\n\n' +
    'Subcommands:\n' +
...
export function createModelCommands(container: ServiceContainer): Command[] {
   ...
   */
  async function modelPullHandler(
    args: string[],
   ...
   */
  async function modelDeleteHandler(
    args: string[],
    service: ModelManagementService
  ): Promise<CommandResult> {
    if (args.length === 0) {
   ...
   */
  async function modelInfoHandler(
    args: string[],
    service: ModelManagementService
  ): Promise<CommandResult> {
    if (args.length === 0) {
    ...
    try {
      ...
      const modifiedAt =
        modifiedAtRaw instanceof Date
          ? modifiedAtRaw
          : modifiedAtRaw
            ? new Date(modifiedAtRaw)
            : undefined;
   ...
   */
  async function modelKeepHandler(
    args: string[],
    service: ModelManagementService
  ): Promise<CommandResult> {
    if (args.length === 0) {
   ...
   */
  async function modelUnloadHandler(
    args: string[],
    service: ModelManagementService
  ): Promise<CommandResult> {
    if (args.length === 0) {
...
export const modelCommands: Command[] = [
  {
    ...
    handler: async (args: string[]): Promise<CommandResult> => {
      ...
      return {
        success: false,
        message:
          'Model management service not initialized. This command requires a service container.',
      };
    },
  },
];
