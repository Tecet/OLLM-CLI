      ...
      await sendToLLM(
        ...
        (error: string) => {
          ...
          try { 
            ...
            if (contextActions?.clearInflightTokens) { contextActions.clearInflightTokens(); }
        ...
        (metrics?: ProviderMetrics, _finishReason?: 'stop' | 'length' | 'tool') => {
          ...
          try { 
            ...
            inflightTokenAccumulatorRef.current = 0;
            if (pending > 0 && contextActions?.reportInflightTokens) { contextActions.reportInflightTokens(pending); }
            if (contextActions?.clearInflightTokens) { contextActions.clearInflightTokens(); }
          } catch (_e) { /* ignore */ }
        },
