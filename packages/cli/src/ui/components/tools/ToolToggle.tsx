 ...
 */
export function ToolToggle({ isEnabled, isSelected, theme }: ToolToggleProps) {
  // Toggle styles
  const enabledIcon = '✓';
  const disabledIcon = '✗';
  ...
  return (
    <Box>
      <Text color={isSelected ? theme.text.accent : theme.text.secondary}>{leftBracket}</Text>
      <Text color={color} bold={isSelected}>
        {icon}
      </Text>
      <Text color={isSelected ? theme.text.accent : theme.text.secondary}>{rightBracket}</Text>
    </Box>
  );
}
