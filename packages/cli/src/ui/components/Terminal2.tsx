 ...
 * Centers the terminal content within the available side panel width.
 * Borders are disabled to use container boundaries.
 */
...
export function Terminal2({ height }: Terminal2Props) {
  ...
  const width = useMemo(() => {
    ...
    // Padding (4) [Border removed]
    // Plus 6 char safety buffer
    return Math.max(10, rightPanelWidth - 10);
  }, [stdout]);

  // Gain back vertical space from borders
