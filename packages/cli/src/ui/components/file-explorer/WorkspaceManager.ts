...
export class WorkspaceManager {
  ...
  loadWorkspace(workspacePath: string): WorkspaceConfig {
    ...
    for (const project of rawConfig.projects) {
      ...
      if (!fs.statSync(projectPath).isDirectory()) {
        if (!this.silent) {
          console.warn(
            `Skipping project "${project.name}" - path is not a directory: ${projectPath}`
          );
        }
        continue;
      }
  ...
  getActiveProject(): ProjectConfig | null {
    ...
    const project = this.workspaceConfig.projects.find((p) => p.name === this.activeProjectName);
  ...
  setActiveProject(projectName: string): void {
    ...
    const project = this.workspaceConfig.projects.find((p) => p.name === projectName);
