...
export function HelpPanel({ isOpen, onClose: _onClose }: HelpPanelProps) {
  ...
  return (
    <Box flexDirection="column" borderStyle="double" borderColor="cyan" padding={1} width={70}>
      {/* Header */}
      <Box marginBottom={1}>
      ...
      {categories.map((category, idx) => (
        <Box
          ...
          marginBottom={idx < categories.length - 1 ? 1 : 0}
        >
          {/* Category header */}
      ...
      <Box marginTop={1} borderStyle="single" borderColor="gray" paddingX={1}>
        <Text dimColor>Tip: Use vim-style keys (hjkl) or arrow keys for navigation</Text>
      </Box>
    </Box>
  );
}
