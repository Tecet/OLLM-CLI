...
} from '@ollm/ollm-cli-core/tools/tool-capabilities.js';
...
export function SettingsPanel({ windowWidth }: SettingsPanelProps) {
  ...
  const { activeKeybinds, defaultKeybinds, setUserBind, restoreDefault, checkConflict } =
    useKeybinds();
  ...
  const capabilityProviders = useMemo(() => {
    ...
    mcpState.servers.forEach((server, serverName) => {
      ...
      const capabilities = detectServerCapabilities(
        serverName,
        adaptedTools as unknown as import('@ollm/ollm-cli-core/tools/types.js').Tool[]
      );
  ...
  const getOptionsForSetting = useCallback(
    (settingId: string) => {
      switch (settingId) {
        case 'provider':
  ...
  const currentSettings = useMemo((): SettingItem[] => {
    ...
    switch (currentSection.id) {
      ...
      case 'model': {
        ...
        const profile =
          profileManager.getProfileById(currentModel) || profileManager.findProfile(currentModel);
        ...
        if (modelEntry) {
          const ctxProfile =
            modelEntry.context_profiles?.find((p) => p.size === settings.llm.contextSize) ||
      ...
      case 'options': {
        ...
        const options: SettingItem[] = [
          ...
          },
          {
            id: 'metricsEnabled',
            label: 'Metrics',
            value: (settings.ui.metricsEnabled as boolean) ?? true,
            type: 'toggle',
            category: 'ui',
          },
          {
            id: 'reasoningEnabled',
            label: 'Reasoning',
            value: (settings.ui.reasoningEnabled as boolean) ?? true,
            type: 'toggle',
            category: 'ui',
          },
        ];
      ...
      case 'theme':
        return Object.keys(builtInThemes).map(
          (themeId) =>
            ({
              id: 'theme',
              label: themeId,
  ...
  const handleSwitchColumn = (direction: 'left' | 'right') => {
    ...
    if (direction === 'right' && focusedColumn === 'left' && currentSection) {
      ...
      while (
        firstFocusable < currentSettings.length &&
    ...
    if (setting.type === 'action') {
      ...
      if (setting.category === 'llm') {
        ...
        if (Object.values(ToolCapability).includes(setting.id as ToolCapability)) {
          updateToolRouting(
            `bindings.${setting.id}`,
            nextValue === '(auto)' ? undefined : nextValue
          );
          return;
        }
      ...
      <Box flexGrow={1} overflow="hidden" flexDirection="row" width="100%">
          ...
          <Box flexDirection="column" flexGrow={1} paddingX={1}>
            ...
            <Box>
              <Text
                ...
                color={
                  isOnExitItem && hasFocus ? uiState.theme.text.accent : uiState.theme.text.primary
                }
              >
                ← Exit
              </Text>
            ...
            {sections.map((section, idx) => {
              const isSelected =
                !isOnExitItem && idx === selectedSectionIndex && focusedColumn === 'left';
        ...
        >
          {currentSection ? (
                  ...
                  .map((setting, sliceIdx) => {
                    ...
                    if (setting.type === 'info') {
                      return (
                        <Box
                          key={`info-${idx}`}
                          marginTop={idx > 0 && currentSettings[idx - 1].type !== 'header' ? 1 : 0}
                          paddingLeft={1}
                        >
                          <Box width="30%">
                            <Text
                          ...
                          <Box flexGrow={1} flexShrink={1}>
                            <Text
                              color={
                                setting.muted || setting.muted_value
                                  ? uiState.theme.text.secondary
                                  : uiState.theme.text.primary
                              }
                              wrap="wrap"
                    ...
                    const isCompactChoice =
                      setting.type === 'choice' && prevSetting && prevSetting.type === 'choice';
                    const marginTop =
                      idx > 0 && prevSetting && prevSetting.type !== 'header' && !isCompactChoice
                        ? 1
                        : 0;
            ...
            <Text color={uiState.theme.text.secondary}>Select a section to view settings</Text>
          )}
        </Box>
      </Box>
    </Box>
