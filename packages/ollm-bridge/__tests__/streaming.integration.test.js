...
describe('Streaming Integration Tests', () => {
    ...
    describe('Property 15: Streaming Chunk Concatenation', () => {
        ...
        it('should concatenate all text chunks to produce complete response', async () => {
            ...
            fc.integer({ min: 1, max: 20 }), async (fullText, chunkSize) => {
        ...
        it('should handle empty text chunks gracefully', async () => {
            await fc.assert(fc.asyncProperty(fc.array(fc.string({ maxLength: 50 }), { minLength: 1, maxLength: 20 }), async (chunks) => {
                // Create event sequence with text chunks (including potentially empty ones)
                const events = [
                    ...chunks.map((chunk) => ({ type: 'text', value: chunk })),
        ...
        it('should preserve chunk order during concatenation', async () => {
            await fc.assert(fc.asyncProperty(fc.array(fc.string({ minLength: 1, maxLength: 20 }), {
                minLength: 2,
                maxLength: 10,
            }), async (chunks) => {
    ...
    describe('Property 16: Tool Call Streaming Delivery', () => {
        ...
        it('should deliver tool calls before stream completes', async () => {
            ...
            fc.array(fc.record({
                ...
                args: fc.dictionary(fc.string({ minLength: 1, maxLength: 20 }), fc.oneof(fc.string({ maxLength: 50 }), fc.integer(), fc.boolean())),
            }), { minLength: 1, maxLength: 5 }), async (toolCallSpecs) => {
        ...
        it('should deliver tool calls in order', async () => {
            ...
            }), { minLength: 2, maxLength: 10 }), async (toolCallSpecs) => {
        ...
        it('should handle mixed text and tool call events', async () => {
            await fc.assert(fc.asyncProperty(fc.string({ minLength: 1, maxLength: 100 }), fc.array(fc.record({
                name: fc.constantFrom('get_weather', 'calculate'),
                args: fc.dictionary(fc.string(), fc.string()),
            }), { minLength: 1, maxLength: 3 }), async (textBefore, toolCallSpecs) => {
                // Create tool calls
                const toolCalls = toolCallSpecs.map((spec, index) => ({
                    id: `call_${index}`,
                    name: spec.name,
                ...
                const events = [
                    ...
                    ...toolCalls.map((tc) => ({ type: 'tool_call', value: tc })),
    ...
    describe('Streaming Edge Cases', () => {
        ...
        it('should handle finish event with different reasons', async () => {
            const finishReasons = ['stop', 'length', 'tool'];
...
//# sourceMappingURL=streaming.integration.test.js.map