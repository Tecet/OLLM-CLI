...
export class MemoryDumpTool implements DeclarativeTool<MemoryDumpParams, ToolResult> {
  ...
  schema: ToolSchema = {
    name: 'write_memory_dump',
    description: 'Writes a "brain dump" of your internal reasoning. Use this ONLY when you are in a specialized analysis mode (Debugger, Security, Planning) and need to offload complex context. NEVER use this tool in Assistant mode, on startup, or for simple greetings.',
    parameters: {
      type: 'object',
      properties: {
        content: {
...
export class MemoryDumpInvocation implements ToolInvocation<MemoryDumpParams, ToolResult> {
  constructor(public params: MemoryDumpParams, private modeManager?: PromptModeManager) {}
  ...
  async execute(
    ...
    try {
      // Safety check: Prevent memory dump in assistant context
       ...
       if (this.params.content.length < 20 && (this.params.content.toLowerCase().includes('hi') || this.params.content.toLowerCase().includes('hello'))) {
           return {
