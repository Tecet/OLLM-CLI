...
describe('WriteFileTool', () => {
  ...
  beforeEach(async () => {
    ...
    testDir = path.join(
      ollmTempDir,
      `test-write-${Date.now()}-${Math.random().toString(36).slice(2)}`
    );
  ...
  describe('execute', () => {
    it('should write content to file', async () => {
      ...
      const invocation = tool.createInvocation({ path: testFile, content: 'Hello World' }, context);
    ...
    it('should create parent directories', async () => {
      ...
      const invocation = tool.createInvocation({ path: testFile, content: 'Hello' }, context);
