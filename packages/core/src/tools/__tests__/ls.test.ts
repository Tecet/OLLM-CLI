...
describe('LsTool', () => {
  ...
  beforeEach(async () => {
    ...
    testDir = path.join(
      ollmTempDir,
      `test-ls-${Date.now()}-${Math.random().toString(36).slice(2)}`
    );
  ...
  describe('execute', () => {
    it('should list directory contents', async () => {
      ...
      const invocation = tool.createInvocation({ path: testDir }, context);
    ...
    it('should handle empty directory', async () => {
      ...
      const context = createToolContext(mockMessageBus);
      const invocation = tool.createInvocation({ path: emptyDir }, context);
      const result = await invocation.execute(abortSignal);
    ...
    it('should handle permission denied', async () => {
      ...
      const context = createToolContext(mockMessageBus);
      const invocation = tool.createInvocation({ path: protectedDir }, context);
      const result = await invocation.execute(abortSignal);
