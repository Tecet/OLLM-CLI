...
export class MCPSearchProvider implements SearchProvider {
  constructor(
    ...
    private capability: ToolCapability.WEB_SEARCH | ToolCapability.DOCUMENTATION_SEARCH
  ) {}
  ...
  async search(query: string, numResults: number): Promise<SearchResult[]> {
    ...
    if (!serverName) {
      ...
      throw new Error(
        `No MCP server available for ${this.capability}. Please enable a compatible extension.`
      );
    }
    ...
    const tools = await (
      client as { getTools: (serverName: string) => Promise<Array<{ name: string }>> }
    ).getTools(serverName);
    const searchTool = tools.find(
      (t: { name: string }) =>
        t.name.toLowerCase().includes('search') || t.name.toLowerCase().includes('query')
    );
  ...
  private parseResults(result: unknown): SearchResult[] {
    ...
    if (Array.isArray(content)) {
      ...
      return content.map((item: unknown) => ({
        ...
        snippet:
          (item as { snippet?: string; description?: string }).snippet ||
          (item as { snippet?: string; description?: string }).description ||
          JSON.stringify(item),
      }));
    }
...
export class DocumentationSearchTool implements DeclarativeTool<
...
class DocSearchInvocation implements ToolInvocation<DocumentationSearchParams, ToolResult> {
  constructor(
    public params: DocumentationSearchParams,
    private router: ToolRouter
  ) {}
  ...
  async execute(signal: AbortSignal): Promise<ToolResult> {
    if (signal.aborted) {
      return {
        llmContent: '',
        returnDisplay: '',
        error: { message: 'Cancelled', type: 'CancelledError' },
      };
    }
