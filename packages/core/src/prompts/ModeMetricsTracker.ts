...
export interface SerializableMetrics {
  ...
  debuggerMetrics: Omit<DebuggerModeMetrics, 'commonErrorTypes'> & {
    commonErrorTypes: Array<[string, number]>;
  };
  planningMetrics: PlanningModeMetrics;
  developerMetrics: DeveloperModeMetrics;
...
export class ModeMetricsTracker {
  ...
  trackModeTransition(transition: ModeTransition): void {
    ...
    const totalConfidence =
      transitionMetrics.averageConfidence * (transitionMetrics.count - 1) + confidence;
  ...
  trackEvent(event: ModeEvent): void {
    switch (event.type) {
      ...
      case 'debugger:fix-applied': {
        ...
        const totalTime =
          this.metrics.debuggerMetrics.averageTimeToFix *
          (this.metrics.debuggerMetrics.fixesApplied - 1);
        this.metrics.debuggerMetrics.averageTimeToFix =
          (totalTime + event.timeToFix) / this.metrics.debuggerMetrics.fixesApplied;
      ...
      case 'tool:tool-executed': {
        ...
        const totalExecutionTime =
          this.metrics.toolMetrics.averageExecutionTime *
          (this.metrics.toolMetrics.toolsExecuted - 1);
        this.metrics.toolMetrics.averageExecutionTime =
          (totalExecutionTime + event.executionTime) / this.metrics.toolMetrics.toolsExecuted;
        break;
      }
    }
  }
      ...
      if (entryTime <= now) {
        ...
        if (timeMetrics) {
          ...
          if (tempModeMetrics.entryCount > 0) {
            tempModeMetrics.averageDuration =
              tempModeMetrics.totalDuration / tempModeMetrics.entryCount;
          }
  ...
  getProductivitySummary(): {
    ...
    return {
      ...
      totalLines: metrics.developerMetrics.linesAdded + metrics.developerMetrics.linesRemoved,
