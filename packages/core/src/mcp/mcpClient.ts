...
export class DefaultMCPClient implements MCPClient {
  ...
  private config: Required<Omit<MCPConfig, 'servers'>> & {
  ...
  constructor(
    ...
    oauthProvider?: MCPOAuthProvider,
    transportFactory?: (config: MCPServerConfig, accessToken?: string) => MCPTransport
  ) {
    this.config = {
  ...
  async startServer(name: string, config: MCPServerConfig): Promise<void> {
    ...
    if (config.oauth?.enabled) {
      ...
      try {
        ...
        throw new Error(
          `OAuth authentication failed for server '${name}': ${error instanceof Error ? error.message : String(error)}`
        );
      }
    }
    ...
    const transport = this.transportFactory
      ? this.transportFactory(config, accessToken)
      : this.createTransport(config, accessToken);
  ...
  getServerStatus(name: string): MCPServerStatus {
    ...
    const uptime =
      state.status === 'connected' && state.startTime ? Date.now() - state.startTime : 0;
  ...
  async getResources(serverName: string): Promise<MCPResource[]> {
    ...
    try {
      ...
      const resources =
        (response.result as { resources?: MCPResource[] } | undefined)?.resources || [];
