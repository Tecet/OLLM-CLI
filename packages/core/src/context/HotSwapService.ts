...
export class HotSwapService {
  ...
  async swap(
    ...
    if (!preserveHistory && this.modeManager && this.snapshotManager && userMessages.length >= 3) {
      try {
        ...
        const snapshot = this.snapshotManager.createTransitionSnapshot(currentMode, targetMode, {
    ...
    if (!preserveHistory && userMessages.length >= 3) {
      ...
      console.log(
        `[HotSwap] Skipping XML snapshot (preserveHistory=${preserveHistory}, messages=${userMessages.length}).`
      );
    }
  ...
  private async generateSnapshot(messages: ContextMessage[]): Promise<string> {
    ...
    if (modelCaps?.reasoning) {
      systemPrompt =
        '[REASONING MODE] Preserve internal reasoning traces enclosed in <think>...</think>. Include them in a <reasoning_traces> section of the snapshot.\n\n' +
        systemPrompt;
    }
    ...
    try {
      ...
      throw new Error(
        `Failed to generate state snapshot: ${err instanceof Error ? err.message : String(err)}`
      );
    }
