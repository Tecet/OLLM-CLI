...
function main() {
  ...
  for (const [tier, results] of Object.entries(byTier).sort()) {
    ...
    console.log('\n📄 Template Only:');
    console.log('─'.repeat(80));
    console.log('Mode'.padEnd(15), 'Tokens'.padEnd(10), 'Usage'.padEnd(10), 'Available'.padEnd(12), 'Status');
    console.log('─'.repeat(80));
    
    for (const result of results.sort((a, b) => a.mode.localeCompare(b.mode))) {
      const status = result.templateTokens > budget ? '❌ EXCEEDS' : 
                     result.templateUsage >= 80 ? '⚠️  High' : '✅ Good';
    ...
    console.log('─'.repeat(80));
    console.log('Mode'.padEnd(15), 'Total'.padEnd(10), 'Usage'.padEnd(10), 'Available'.padEnd(12), 'Status');
    console.log('─'.repeat(80));
