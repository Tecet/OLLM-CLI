...
const mockTools = [{ name: 'read_file' }, { name: 'write_file' }, { name: 'list_directory' }];
...
async function reproduce() {
  ...
  const modeManager = new PromptModeManager(systemPromptBuilder, promptRegistry, contextAnalyzer);
  ...
  try {
    const result = modeManager.validateToolArgsForPlanningMode('write_file', {
      path: 'src/test.ts',
    });
