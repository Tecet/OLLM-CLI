...
async function testSearch() {
  ...
  try {
    const response = await fetch(
      'https://registry.modelcontextprotocol.io/v0/servers?q=context7&limit=10',
      {
        headers: {
  ...
  try {
    const response = await fetch(
      'https://registry.modelcontextprotocol.io/v0/servers?q=brave&limit=10',
      {
        headers: {
          Accept: 'application/json',
          'User-Agent': 'OLLM-CLI/0.1.0',
  ...
  try {
    const response = await fetch(
      'https://registry.modelcontextprotocol.io/v0/servers/io.github.upstash%2Fcontext7',
      {
        headers: {
          Accept: 'application/json',
          'User-Agent': 'OLLM-CLI/0.1.0',
    ...
    if (response.ok) {
      ...
      console.log(
        `   Env vars: ${server.packages?.[0]?.environmentVariables?.map((v) => v.name).join(', ')}`
      );
    } else {
