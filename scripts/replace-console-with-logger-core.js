...
const loggerRelTarget = path.join('..', 'utils', 'logger.js'); // relative from src subfolders or adjust
...
function processFile(filePath) {
  ...
  const createLoggerImportRegex = /createLogger\s*\{/;
  const hasCreateLoggerImport = /createLogger\s*\(|from\s+['"].*logger['"]/m.test(content) || /createLogger\(/.test(content);
  // Simple check for import from logger
  const hasImportFromLogger = new RegExp("from \\\"" + importPath.replace(/[-\\/\\^$*+?.()|[\]{}]/g, '\\$&') + "\\\"","m");

  // Add import if missing
  if (!/from\s+['"].*logger['"]/m.test(content)) {
    // Insert after first existing import block or at top
    const importStmt = `import { createLogger } from '${importPath}';\n`;
    if (/^\s*import .* from .*;?/m.test(content)) {
      // find last import
      const lastImport = content.match(/(^\s*import[\s\S]*?;?\n)/m);
      // naive: insert at top
  ...
  if (!/const\s+logger\s*=\s*createLogger\(/m.test(content)) {
    ...
    // safer: place after first import block
    const importBlockMatch = content.match(/(^[\s\S]*?\n)(?!import)/);
    // just insert after the first import statements match
